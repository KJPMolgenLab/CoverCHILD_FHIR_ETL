...
summarise_mean_sd <- function(df) {
  summarise(df, 
            ...
            across(c(age_adm, length_stay_brutto, n_icd_f, n_icd_other),
...
### counts & proportions for lockdowns
```{r}
df_ed %>% gen_tabyl(covid_lockd, icd_f50)
df_ed %>% gen_tabyl(icd_f50, sex, covid_lockd)
```

### counts & proportions for lockdowns & ED subcategories
```{r}
...
## klinische Präsentation während Pandemie
### jüngeres Alter
```{r echo=TRUE}
