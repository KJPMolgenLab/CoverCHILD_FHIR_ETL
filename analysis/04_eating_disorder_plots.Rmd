...
df_ed_share_month %>% 
  ...
  geom_area(aes(fill = f50_prop_type)) +
...
df_ed_share_month_yearly %>%
  ...
  scale_colour_discrete(type = set_names(gu_colours$main[c(1, 3:5)], NULL)) +
  guides(x = guide_axis(angle = -90)) + 
  # labs(title = "...",
  #      x = "month", y = "... [%]") +
  theme_classic()
...
df_ed_share_re_adm %>%
  ...
  scale_colour_discrete(type = set_names(gu_colours$main[c("gu_emo_rot", "gu_goethe_blau")], NULL),
                        name = "ED Diagnosis", labels = c("no ED", "ED")) +
  # geom_line(aes(colour = f50_prop_type)) +
  guides(x = guide_axis(angle = -90)) +
