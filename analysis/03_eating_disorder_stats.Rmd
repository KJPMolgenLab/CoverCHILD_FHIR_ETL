...
f50_types <- list(
  c("F50.0", "F50.00", "F50.01", "F50.1") ~ "AN",
  c("F50.2", "F50.3") ~ "BN",
  c("F50.4", "F50.5", "F50.8", "F50.9", "F98.2") ~ "Essstörung sonst."
  )

# create ED category variables
df_an %<>% 
  mutate(icd_f50 = factor(if_else(str_detect(as.character(icd_code), "F50|F98.2"), "F50+", "F50-"), 
                          levels = c("F50-", "F50+"), ordered = TRUE),
         f50_type = factor(case_match(as.character(icd_code), !!!f50_types, .default = "F50-"),
                           levels = c("F50-", "Essstörung sonst.", "BN", "AN"), ordered = TRUE),
         covid = factor(if_else(adm_date < covid_start, "pre_covid", "dur_covid"), 
                        levels = c("pre_covid", "dur_covid"), ordered = TRUE))

# exclude cases with multiple ED subdiagnosis
cases_excl_mult_ed <- df_an %>% 
  filter(icd_f50 == "F50+") %>%
  group_by(case_id) %>%
  filter(n_distinct(f50_type) > 1) %>% 
  use_series(case_id) %>% 
  as.character() %>% 
  unique()
...
df_c <- df_an %>% 
  ...
  # filter(!(case_id %in% cases_excl_mult_ed)) %>%
  group_by(case_id) %>%
  mutate(icd_f50 = max(icd_f50), 
         f50_type = max(f50_type)) %>%
...
df_c %>% 
  ...
  summarise(#F50_prop = sum(icd_f50 == "F50+")/n()*100, 
            AN_prop = sum(f50_type == "AN")/n()*100,
            BN_prop = sum(f50_type == "BN")/n()*100,
            ES_sonst_prop = sum(f50_type == "Essstörung sonst.")/n()*100) %>% 
...
df_c %>% 
  ...
  ungroup() %>%
  # slice_head(n = 500) %>%
  ggplot(aes(x = adm_date, xend = dis_date, y = p_id, yend = p_id, group = case_id, colour = f50_type)) +
  geom_segment() + 
...
```{r}
df_c %>% gen_tabyl(covid, f50_type)
df_c %>% gen_tabyl(f50_type, sex)
df_c %>% gen_tabyl(f50_type, sex, covid)
```
