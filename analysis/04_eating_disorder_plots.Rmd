...
ed_ggsave <- function(filename, path = plotdir, device = "tiff", scale = 1, dpi = 300, ...) {
...
```{r}
school_closure_label <- "School closure periods"
school_closure_severity_labels <- c("1" = "partial", "2" = "full (incl. holidays)")
covid_phase_labels <- list(NULL = "1", "1st school\nclosure" = "2", "1st open period" = "3",
                           "2nd school closure" = "4", "2nd open period" = "5")
```
...
{df_ed_covid_weekly_long %>% 
    ...
    scale_x_discrete(
      breaks = 
        levels(df_ed_covid_weekly_long$adm_year_week)[seq(1, length(levels(df_ed_covid_weekly_long$adm_year_week)), 2)],
...
{df_ed %>%
    ...
    geom_text(aes(x = "2020.3", y = 100, label = covid_onset_label, hjust = "left", vjust = "top"),
