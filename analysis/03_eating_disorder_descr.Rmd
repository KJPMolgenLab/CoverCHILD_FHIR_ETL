...
```{r}
#TODO include above
df_ed %>% gen_tabyl(covid_lockd_sev, icd_f50)
df_ed %>% gen_tabyl(icd_f50, sex, covid_lockd_sev)
```

### sex counts & proportions for lockdowns & ED subcategories
```{r}
df_ed %>% gen_tabyl(covid_lockd_sev, f50_type)
df_ed %>% gen_tabyl(f50_type, sex, covid_lockd_sev)
```

### by covid phase
```{r}
df_ed %>% gen_tabyl(covid_period_i, icd_f50)
df_ed %>% gen_tabyl(covid_period_i, f50_type)
df_ed %>% gen_tabyl(covid_period_i, icd_f50_dd_ad)
```
...
df_ed %>% gen_tabyl(covid_lockd_sev, icd_f50_dd_ad)
```
#### LOS
```{r}
df_ed %>%
...
### within 6 months
```{r}
df_ed %>% gen_tabyl(icd_f50, re_adm_soon_nona, covid_pan)
```
